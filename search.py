...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state): 
            return node
        if ''.join(node.state) not in closed:
            closed[''.join(node.state)] = True
            fringe.extend(node.expand(problem))    
    return None
...
class InstrumentedProblem(Problem):
    ...
    def __repr__(self):
		""" print for each test case these 3 pieces of information
			self.succs: total number of times the successor function is called
			self.goal_tests: total number of nodes expanded  --> 
			self.states:  total number of nodes generated  -- time complexity
			"""
		return '(%4d %4d %4d)' % (self.succs, self.goal_tests, self.states)
...
def compare_searchers(problems, header, searchers=[breadth_first_tree_search,
                      ...
                      iterative_deepening_search, depth_limited_search, astar_search]):
